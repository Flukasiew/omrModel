...
class CTC_PriMuS:
    ...
    def nextBatch(self, params):
        ...
        for _ in range(params['batch_size']):
            ...
            print(sample_filepath, sample_fullpath)

            # IMAGE
            if self.distortions:
                sample_img = cv2.imread(sample_fullpath + '_distorted.jpg', False) # Grayscale is assumed
            else:
                sample_img = cv2.imread(sample_fullpath + '.jpg', False)  # Grayscale is assumed!
            ...
            # GROUND TRUTH
            if self.semantic:
                sample_full_filepath = sample_fullpath + ".txt"#'.semantic'
            else:
